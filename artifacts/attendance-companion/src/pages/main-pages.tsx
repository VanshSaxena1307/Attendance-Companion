...
function MetricCard({ label, value, note, tone = 'teal', icon: Icon }: { label:string; value:string|number; note:string; tone?:string; icon: typeof Target }) {
  return <div className="group relative overflow-hidden rounded-2xl border border-card-border bg-card p-5 shadow-[0_5px_18px_hsl(191_35%_17%/.04)] transition-transform hover:-translate-y-0.5"><div className={`absolute right-0 top-0 h-20 w-20 translate-x-7 -translate-y-7 rounded-full ${tone === 'amber' ? 'bg-accent/40' : tone === 'rose' ? 'bg-[#f4d8cf]' : 'bg-[#d9e9df]'}`}/><div className="relative flex items-start justify-between"><p className="text-[11px] font-bold uppercase tracking-[.15em] text-muted-foreground">{label}</p><Icon size={18} className="text-primary/70"/></div><p data-testid={`metric-${label.toLowerCase().replaceAll(' ','-')}`} className="relative mt-5 font-mono text-3xl tracking-tight">{value}</p><p className="relative mt-1 text-xs text-muted-foreground">{note}</p></div>;
}
...
function SubjectRow({ subject, compact=false }: { subject: SubjectAttendance; compact?: boolean }) {
  return <Link href="/attendance" data-testid={`link-subject-${subject.id}`} className="group block rounded-2xl border border-border/70 bg-card p-4 transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-3"><span className="mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-xl text-[10px] font-bold" style={{background:`${subject.color}22`,color:subject.color}}>{subject.code.slice(0,2)}</span><div className="min-w-0"><p className="truncate text-sm font-semibold">{subject.name}</p><p className="mt-0.5 truncate text-xs text-muted-foreground">{subject.code} · {subject.teacher}</p></div></div><div className="text-right"><p className="font-mono text-xl font-bold">{pct(subject.percentage)}</p><StatusPill status={subject.status}/></div></div><div className="mt-4 flex items-center gap-3"><ProgressBar value={subject.percentage} target={subject.target} color={subject.color}/><span className="shrink-0 font-mono text-[10px] text-muted-foreground">{subject.present}/{subject.total}</span></div>{!compact && <p className="mt-2 text-[11px] text-muted-foreground">{subject.percentage >= subject.target ? `${(subject.percentage-subject.target).toFixed(1)} points above target` : `${(subject.target-subject.percentage).toFixed(1)} points to target`}</p>}</Link>;
}
...
function Dashboard({ user }: { user: CurrentUser }) {
  ...
  return <AppShell user={user}><PageHeader eyebrow={`${new Intl.DateTimeFormat('en-IN',{weekday:'long',day:'numeric',month:'long'}).format(new Date())}`} title={user.role === 'STUDENT' ? `Good morning, ${user.name.split(' ')[0]}.` : 'Good morning.'} description={user.role === 'STUDENT' ? 'Here is the clearest view of where your attendance stands.' : 'A quick read on the students who need your attention.'} action={<Link href="/attendance" data-testid="link-view-attendance" className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-2.5 text-[13px] font-semibold hover:border-primary/40 hover:bg-secondary">View full attendance <ChevronRight size={15}/></Link>}/><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 animate-rise-in delay-1"><MetricCard label="Overall attendance" value={pct(data.overall.percentage)} note={`${data.overall.present} of ${data.overall.total} classes present`} icon={Target}/><MetricCard label="Safe subjects" value={`${data.subjects.filter(s=>s.status==='SAFE').length}/${data.subjects.length}`} note="At or above your target" icon={ShieldCheck}/><MetricCard label="Need attention" value={data.subjects.filter(s=>s.status!=='SAFE').length} note="Subjects below target" tone="amber" icon={CircleAlert}/><MetricCard label="Open actions" value={data.pendingRequests + data.openIssues} note={`${data.pendingRequests} requests · ${data.openIssues} issues`} tone="rose" icon={FilePlus2}/></div>{user.role === 'STUDENT' && <StudentTodaySchedule />}<div className="mt-6 grid gap-6 xl:grid-cols-[1.35fr_.65fr]"><section className="rounded-2xl border border-border/70 bg-card p-5 sm:p-6"><div className="mb-5 flex items-end justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[.15em] text-primary">Subject pulse</p><h2 className="mt-1 font-display text-2xl">Your classes, at a glance</h2></div><Link href="/attendance" data-testid="link-subject-pulse" className="text-xs font-semibold text-primary hover:underline">See all</Link></div><div className="grid gap-3 sm:grid-cols-2">{data.subjects.slice(0,4).map((s)=><SubjectRow key={s.id} subject={s} compact/>)}</div></section><section className="relative overflow-hidden rounded-2xl bg-primary p-6 text-primary-foreground"><div className="absolute -right-12 -top-12 h-40 w-40 rounded-full border-[20px] border-accent/20"/><Sparkles size={20} className="text-accent"/><p className="mt-5 text-[11px] font-bold uppercase tracking-[.15em] text-primary-foreground/60">A useful read</p><h2 data-testid="text-dashboard-insight" className="mt-2 font-display text-[27px] leading-tight">{data.insight.headline}</h2><p className="mt-3 text-sm leading-6 text-primary-foreground/75">{data.insight.detail}</p><div className="mt-7 grid grid-cols-2 gap-3 border-t border-primary-foreground/15 pt-4"><div><p className="font-mono text-2xl text-accent">{data.insight.classesToTarget}</p><p className="text-[11px] text-primary-foreground/60">classes to target</p></div><div><p className="font-mono text-2xl">{data.insight.canMiss}</p><p className="text-[11px] text-primary-foreground/60">classes you can miss</p></div></div><Link href="/insights" data-testid="link-read-insights" className="mt-6 inline-flex items-center gap-2 text-xs font-bold text-accent hover:underline">Read the full insight <ArrowUpRight size={14}/></Link></section></div><div className="mt-6 grid gap-6 xl:grid-cols-[.9fr_1.1fr]"><section className="rounded-2xl border border-border/70 bg-card p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[.15em] text-primary">Next steps</p><h2 className="mt-1 font-display text-2xl">Keep things moving</h2></div><Clock3 size={19} className="text-muted-foreground"/></div><div className="space-y-1">{data.pendingRequests > 0 && <Link href="/requests" data-testid="link-pending-requests" className="flex items-center gap-3 rounded-xl p-3 hover:bg-muted"><span className="grid h-8 w-8 place-items-center rounded-lg bg-accent/40"><ClipboardCheck size={15}/></span><span className="flex-1 text-sm">Review your exemption requests<p className="text-xs text-muted-foreground">{data.pendingRequests} awaiting a decision</p></span><ChevronRight size={16} className="text-muted-foreground"/></Link>}{data.openIssues > 0 && <Link href="/issues" data-testid="link-open-issues" className="flex items-center gap-3 rounded-xl p-3 hover:bg-muted"><span className="grid h-8 w-8 place-items-center rounded-lg bg-[#f4d8cf]"><FileWarningIcon/></span><span className="flex-1 text-sm">Check attendance issues<p className="text-xs text-muted-foreground">{data.openIssues} need a follow-up</p></span><ChevronRight size={16} className="text-muted-foreground"/></Link>}<Link href="/insights" data-testid="link-weekly-insights" className="flex items-center gap-3 rounded-xl p-3 hover:bg-muted"><span className="grid h-8 w-8 place-items-center rounded-lg bg-[#d9e9df]"><TrendingUp size={15}/></span><span className="flex-1 text-sm">See your attendance trend<p className="text-xs text-muted-foreground">Find the classes shaping your average</p></span><ChevronRight size={16} className="text-muted-foreground"/></Link></div></section><section className="rounded-2xl border border-border/70 bg-card p-5 sm:p-6"><div className="mb-4 flex items-center justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[.15em] text-primary">Recent activity</p><h2 className="mt-1 font-display text-2xl">The paper trail</h2></div><Link href="/notifications" data-testid="link-all-activity" className="text-xs font-semibold text-primary hover:underline">All activity</Link></div><div className="space-y-0">{safeArray(data.recentActivity).slice(0,4).map((a,i)=><div key={a.id} data-testid={`activity-${a.id}`} className="flex gap-3 border-b border-border/60 py-3 last:border-0"><span className={`mt-1 h-2 w-2 shrink-0 rounded-full ${a.type==='ISSUE'?'bg-destructive':a.type==='WARNING'?'bg-accent':'bg-primary'}`}/><div className="min-w-0 flex-1"><p className="text-sm font-medium">{a.title}</p><p className="mt-0.5 truncate text-xs text-muted-foreground">{a.description}</p></div><span className="shrink-0 text-[10px] text-muted-foreground">{a.time}</span></div>)}</div></section></div></AppShell>;
}
...
function Attendance({ user }: { user: CurrentUser }) {
  ...
  return <AppShell user={user}><PageHeader eyebrow="Attendance" title="Know where you stand." description="A transparent view of every subject and every marked class. Your target is the line, not a judgment." action={<Link href="/issues" data-testid="link-report-from-attendance" className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-[13px] font-semibold text-primary-foreground hover:brightness-110"><FilePlus2 size={15}/> Report a discrepancy</Link>}/><div className="mb-6 flex gap-2 overflow-x-auto pb-1">{['All subjects', ...subjects.map(s=>s.code)].map((item,i)=><button key={item} onClick={()=>setSubject(i===0?'':subjects[i-1]?.id || '')} data-testid={`button-filter-subject-${i}`} className={`whitespace-nowrap rounded-full border px-3.5 py-2 text-xs font-semibold transition-colors ${(!subject&&i===0)||(subject&&item===subjects.find(s=>s.id===subject)?.code) ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-card text-muted-foreground hover:bg-muted'}`}>{item}</button>)}</div><div className="grid gap-4 md:grid-cols-2">{subjectQuery.isLoading ? <LoadingBlock rows={4}/> : subjects.map(s=><SubjectRow key={s.id} subject={s}/>)}</div><section className="mt-7 rounded-2xl border border-border/70 bg-card p-5 sm:p-6"><div className="mb-5 flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[.15em] text-primary">Date-wise history</p><h2 className="mt-1 font-display text-2xl">What was marked</h2></div><div className="flex flex-wrap gap-2"><select value={status} onChange={e=>setStatus(e.target.value)} data-testid="select-history-status" className="rounded-xl border border-input bg-background px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-ring"><option value="">Every status</option><option value="PRESENT">Present</option><option value="ABSENT">Absent</option><option value="LATE">Late</option><option value="EXEMPTED">Exempted</option></select><Button variant="ghost" onClick={()=>{setSubject('');setStatus('')}} testId="button-clear-filters"><X size={14}/> Clear</Button></div></div>{history.isLoading ? <LoadingBlock rows={5}/> : history.isError ? <ErrorBlock retry={()=>history.refetch()}/> : records.length === 0 ? <EmptyBlock title="No classes match that filter" detail="Try widening the filters to see more of your attendance history."/> : <div className="overflow-x-auto"><table className="w-full min-w-[630px] text-left"><thead><tr className="border-b border-border text-[10px] font-bold uppercase tracking-[.15em] text-muted-foreground"><th className="pb-3">Date</th><th className="pb-3">Subject</th><th className="pb-3">Mark</th><th className="pb-3">Details</th></tr></thead><tbody>{records.map((r,i)=><tr key={`${r.date}-${r.subjectId}-${i}`} data-testid={`row-attendance-${i}`} className="border-b border-border/60 last:border-0"><td className="py-3.5 font-mono text-xs">{fmtDate(r.date)}</td><td className="py-3.5"><p className="text-sm font-semibold">{r.subjectName}</p><p className="text-[11px] text-muted-foreground">{r.subjectCode}</p></td><td className="py-3.5"><StatusPill status={r.status}/></td><td className="py-3.5 text-xs text-muted-foreground">{r.detail}</td></tr>)}</tbody></table></div>}</section></AppShell>;
}
...
function Requests({ user }: { user: CurrentUser }) {
  ...
  return <AppShell user={user}><PageHeader eyebrow="Requests" title="Exemptions, with a paper trail." description="If a college-approved absence should not count against you, start here. Keep supporting details clear and specific." action={user.role==='STUDENT'?<Button onClick={()=>setModal(true)} testId="button-submit-exemption"><Plus size={16}/> Submit request</Button>:undefined}/><div className="mb-5 flex items-center gap-2 overflow-x-auto">{['','PENDING','APPROVED','REJECTED'].map(s=><button key={s||'all'} onClick={()=>setFilter(s)} data-testid={`button-filter-request-${s||'all'}`} className={`rounded-full px-3.5 py-2 text-xs font-semibold ${filter===s?'bg-primary text-primary-foreground':'border border-border bg-card text-muted-foreground hover:bg-muted'}`}>{s||'All requests'}</button>)}</div>{query.isLoading?<LoadingBlock rows={4}/>:query.isError?<ErrorBlock retry={()=>query.refetch()}/>:items.length===0?<EmptyBlock title="No exemption requests yet" detail="Approved college work, medical leave, or competition travel can be documented here." action={user.role==='STUDENT'?<Button onClick={()=>setModal(true)} testId="button-empty-submit"><Plus size={15}/> Start a request</Button>:undefined}/>:<div className="space-y-3">{items.map((x:Exemption)=><div key={x.id} data-testid={`card-exemption-${x.id}`} className="rounded-2xl border border-border/70 bg-card p-5 transition-shadow hover:shadow-sm"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><div className="flex flex-wrap items-center gap-2"><span className="text-[10px] font-bold uppercase tracking-[.14em] text-primary">{x.category.replaceAll('_',' ')}</span><StatusPill status={x.status}/>{user.role!=='STUDENT'&&<span className="text-xs text-muted-foreground">by {x.studentName}</span>}</div><h3 className="mt-2 text-sm font-semibold">{x.reason}</h3><p className="mt-1 text-xs text-muted-foreground">{fmtDate(x.startDate)} — {fmtDate(x.endDate)} {x.proofName&&` · ${x.proofName}`}</p></div><span className="text-[11px] text-muted-foreground">Submitted {shortDate(x.submittedAt)}</span></div>{x.reviewerRemarks&&<p className="mt-4 rounded-xl bg-muted px-3 py-2 text-xs text-muted-foreground"><span className="font-semibold text-foreground">Reviewer note: </span>{x.reviewerRemarks}</p>}{user.role!=='STUDENT'&&x.status==='PENDING'&&<div className="mt-4 flex gap-2 border-t border-border/60 pt-3"><Button onClick={()=>review(x.id,true)} disabled={approve.isPending} testId={`button-approve-${x.id}`}><Check size={14}/> Approve</Button><Button onClick={()=>review(x.id,false)} variant="danger" disabled={reject.isPending} testId={`button-reject-${x.id}`}><X size={14}/> Decline</Button></div>}</div>)}</div>}{modal&&<Modal title="Submit an exemption" close={()=>setModal(false)}><form onSubmit={submit} className="space-y-4"><Field label="Reason category"><select name="category" required data-testid="select-exemption-category" className={inputClass}><option value="MEDICAL">Medical</option><option value="COLLEGE_EVENT">College event</option><option value="OFFICIAL_WORK">Official work</option><option value="SPORTS">Sports</option><option value="COMPETITION">Competition</option><option value="PERSONAL">Personal</option><option value="OTHER">Other</option></select></Field><Field label="What happened?" hint="A short, factual explanation helps reviewers decide quickly."><textarea name="reason" required minLength={1} rows={3} data-testid="textarea-exemption-reason" className={inputClass} placeholder="Tell us what the absence was for…"/></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="From"><input name="startDate" required type="date" data-testid="input-exemption-start" className={inputClass}/></Field><Field label="To"><input name="endDate" required type="date" data-testid="input-exemption-end" className={inputClass}/></Field></div><Field label="Proof filename" hint="Optional. Add the name of the document you will share with your reviewer."><input name="proofName" data-testid="input-exemption-proof" className={inputClass} placeholder="e.g. medical-note.pdf"/></Field><Button type="submit" disabled={create.isPending} className="w-full" testId="button-submit-exemption-form"><Send size={15}/>{create.isPending?'Sending…':'Send for review'}</Button></form></Modal>}</AppShell>;
}
...
function Issues({ user }: { user: CurrentUser }) {
 ...
 return <AppShell user={user}><PageHeader eyebrow="Discrepancies" title="Something look off?" description="Attendance records are data, not destiny. Tell us what needs checking and we will keep the review visible." action={user.role==='STUDENT'?<Button onClick={()=>setModal(true)} testId="button-report-issue"><Plus size={16}/> Report an issue</Button>:undefined}/>{query.isLoading?<LoadingBlock rows={4}/>:query.isError?<ErrorBlock retry={()=>query.refetch()}/>:items.length===0?<EmptyBlock title="No attendance issues" detail="If a class was marked incorrectly or an approved exemption is missing, you can report it here." action={user.role==='STUDENT'?<Button onClick={()=>setModal(true)} testId="button-empty-report"><Plus size={15}/> Report a discrepancy</Button>:undefined}/>:<div className="space-y-3">{items.map((x:AttendanceIssue)=><div key={x.id} data-testid={`card-issue-${x.id}`} className="rounded-2xl border border-border/70 bg-card p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row"><div><div className="flex flex-wrap items-center gap-2"><span className="text-[10px] font-bold uppercase tracking-[.12em] text-primary">{x.issueType.replaceAll('_',' ')}</span><StatusPill status={x.status}/>{user.role!=='STUDENT'&&<span className="text-xs text-muted-foreground">{x.studentName}</span>}</div><h3 className="mt-2 text-sm font-semibold">{x.subjectName} <span className="font-normal text-muted-foreground">· {fmtDate(x.date)}</span></h3><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">{x.description}</p></div><span className="text-[11px] text-muted-foreground">{shortDate(x.createdAt)}</span></div>{x.evidenceName&&<p className="mt-3 text-xs text-primary">Evidence: {x.evidenceName}</p>}{x.reviewerRemarks&&<p className="mt-4 rounded-xl bg-muted px-3 py-2 text-xs text-muted-foreground"><span className="font-semibold text-foreground">Reviewer note: </span>{x.reviewerRemarks}</p>}{user.role!=='STUDENT'&&['OPEN','UNDER_REVIEW'].includes(x.status)&&<div className="mt-4 flex gap-2 border-t border-border/60 pt-3"><Button onClick={()=>resolve.mutate({id:x.id,data:{remarks:'Attendance record reviewed and corrected.',updateAttendance:true}},{onSuccess:()=>client.invalidateQueries({queryKey:getGetAttendanceIssuesQueryKey()})})} disabled={resolve.isPending} testId={`button-resolve-issue-${x.id}`}><Check size={14}/> Resolve & update</Button><Button onClick={()=>reject.mutate({id:x.id,data:{remarks:'The attendance record matches the source register.'}},{onSuccess:()=>client.invalidateQueries({queryKey:getGetAttendanceIssuesQueryKey()})})} variant="danger" disabled={reject.isPending} testId={`button-reject-issue-${x.id}`}><X size={14}/> Reject</Button></div>}</div>)}</div>}{modal&&<Modal title="Report an attendance issue" close={()=>setModal(false)}><form onSubmit={submit} className="space-y-4"><Field label="Subject"><select name="subjectId" required data-testid="select-issue-subject" className={inputClass}>{subjects.map(s=><option key={s.id} value={s.id}>{s.code} · {s.name}</option>)}</select></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="Class date"><input name="date" required type="date" data-testid="input-issue-date" className={inputClass}/></Field><Field label="Issue type"><select name="issueType" required data-testid="select-issue-type" className={inputClass}><option value="INCORRECTLY_MARKED_ABSENT">Marked absent incorrectly</option><option value="INCORRECTLY_MARKED_PRESENT">Marked present incorrectly</option><option value="EXEMPTION_NOT_REFLECTED">Exemption not reflected</option><option value="WRONG_ATTENDANCE_DATA">Wrong attendance data</option><option value="OTHER">Other</option></select></Field></div><Field label="What should we check?"><textarea name="description" required rows={4} data-testid="textarea-issue-description" className={inputClass} placeholder="Explain what happened and what the record should say…"/></Field><Field label="Evidence filename"><input name="evidenceName" data-testid="input-issue-evidence" className={inputClass} placeholder="Optional document or screenshot name"/></Field><Button type="submit" disabled={create.isPending} className="w-full" testId="button-submit-issue-form"><Send size={15}/>{create.isPending?'Sending…':'Send report'}</Button></form></Modal>}</AppShell>;
}
...
function Insights({ user }: { user: CurrentUser }) {
 ...
 return <AppShell user={user}><PageHeader eyebrow="Insights" title="Make the next class count." description="A steady, deterministic read of your trend and the small choices that protect your target."/><div className="grid gap-4 sm:grid-cols-3"><MetricCard label="Current average" value={summary.data?pct(summary.data.overall.percentage):'—'} note={summary.data?`Target is ${summary.data.overall.target}%`:'Loading your average'} icon={Target}/><MetricCard label="Strongest subject" value={best?.code||'—'} note={best?`${pct(best.percentage)} · ${best.name}`:'No data yet'} icon={ArrowUpRight}/><MetricCard label="Watch closely" value={risk?.code||'—'} note={risk?`${pct(risk.percentage)} · ${risk.name}`:'No data yet'} tone="amber" icon={ArrowDownRight}/></div><div className="mt-6 grid gap-6 xl:grid-cols-[1.4fr_.6fr]"><section className="rounded-2xl border border-border/70 bg-card p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[.15em] text-primary">Trend</p><h2 className="mt-1 font-display text-2xl">Your attendance over time</h2></div><span className="rounded-full bg-secondary px-3 py-1.5 text-[11px] font-semibold text-secondary-foreground">Target line included</span></div>{trend.isLoading?<div className="h-[280px] animate-pulse rounded-xl bg-muted"/>:points.length===0?<EmptyBlock title="Your trend is taking shape" detail="More marked classes will unlock a clearer pattern."/>:<div className="h-[280px] w-full"><ResponsiveContainer width="100%" height="100%"><AreaChart data={points}><defs><linearGradient id="trendFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#2f7665" stopOpacity=".26"/><stop offset="100%" stopColor="#2f7665" stopOpacity=".01"/></linearGradient></defs><CartesianGrid strokeDasharray="3 5" vertical={false} stroke="hsl(var(--border))"/><XAxis dataKey="label" tick={{fontSize:11,fill:'hsl(var(--muted-foreground))'}} axisLine={false} tickLine={false}/><YAxis domain={[50,100]} tick={{fontSize:11,fill:'hsl(var(--muted-foreground))'}} axisLine={false} tickLine={false} unit="%"/><Tooltip contentStyle={{borderRadius:12,border:'1px solid hsl(var(--border))',background:'hsl(var(--card))',fontSize:12}}/><Area type="monotone" dataKey="percentage" stroke="#2f7665" strokeWidth={3} fill="url(#trendFill)" dot={{fill:'#f4f3ed',stroke:'#2f7665',strokeWidth:2,r:4}}/></AreaChart></ResponsiveContainer></div>}</section><section className="rounded-2xl bg-[#e9e7dc] p-6"><div className="grid h-9 w-9 place-items-center rounded-xl bg-accent"><LightbulbIcon/></div><p className="mt-6 text-[11px] font-bold uppercase tracking-[.15em] text-primary">The honest version</p><h2 className="mt-2 font-display text-[28px] leading-tight">{summary.data?.insight.headline||'Small consistency beats last-minute catching up.'}</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">{summary.data?.insight.detail||'Keep showing up to the classes furthest from target. The maths rewards consistency.'}</p><Link href="/attendance" data-testid="link-insight-attendance" className="mt-6 inline-flex items-center gap-2 text-xs font-bold text-primary hover:underline">Check subject detail <ChevronRight size={14}/></Link></section></div><section className="mt-6 rounded-2xl border border-border/70 bg-card p-5 sm:p-6"><div className="mb-5"><p className="text-[11px] font-bold uppercase tracking-[.15em] text-primary">Subject context</p><h2 className="mt-1 font-display text-2xl">Where your average is made</h2></div><div className="grid gap-3 md:grid-cols-2">{subjects.map(s=><SubjectRow key={s.id} subject={s}/>)}</div></section></AppShell>;
}
