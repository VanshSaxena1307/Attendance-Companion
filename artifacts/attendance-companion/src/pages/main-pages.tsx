...
const fmtDate = (value: string) => {
  if (/^\d{4}-\d{2}-\d{2}$/.test(value)) {
    ...
    return new Intl.DateTimeFormat('en-IN', { timeZone: 'Asia/Kolkata', day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(Date.UTC(y, m - 1, d, 12, 0, 0)));
  }
...
const shortDate = (value: string) => {
  if (/^\d{4}-\d{2}-\d{2}$/.test(value)) {
    const [y, m, d] = value.split('-').map(Number);
    return new Intl.DateTimeFormat('en-IN', { timeZone: 'Asia/Kolkata', day: 'numeric', month: 'short' }).format(new Date(Date.UTC(y, m - 1, d, 12, 0, 0)));
  }
...
export function TeacherAttendance({ user }: { user: CurrentUser }) {
  ...
  // Selected schedule date (defaults to today in Asia/Kolkata)
                  ...
                  <p className="opacity-90">
                    ...
                    {existing.data?.[0]?.markedAt && (
                      <span className="block mt-0.5 text-[11px] opacity-75">
                        Submitted at {new Date(existing.data[0].markedAt).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit', hour12: true })}
                      </span>
                    )}
                  </p>
                </div>
...
function Issues({ user }: { user: CurrentUser }) {
 const [modal,setModal]=useState(false); const client=useQueryClient(); const subjectQuery=useGetSubjectAttendance({ query: { enabled: user.role === 'STUDENT', queryKey: getGetSubjectAttendanceQueryKey() } }); const subjects=safeArray(subjectQuery.data); const query=useGetAttendanceIssues(); const create=useCreateAttendanceIssue(); const resolve=useResolveAttendanceIssue(); const reject=useRejectAttendanceIssue(); const items=safeArray(query.data);
