...
import { db, pool, attendanceTable, sectionsTable, studentsTable, subjectsTable, teacherSubjectSectionsTable, teachersTable, usersTable } from "./index";
...
async function main() {
  ...
  const attendanceBook = XLSX.readFile(path.join(dataDir, "Attendance_Demo.xlsx"), { cellDates: true });
  ...
  const flushAttendance = async () => {
    ...
    await db.insert(attendanceTable).values(attendanceBatch).onConflictDoNothing();
  ...
  for (const row of mappingRows) {
    const teacherCode = text(row[0]); const teacherName = text(row[1]); const teacherId = id("teacher", teacherCode); const teacherEmail = text(row[2]); const mapped = subjectByName.get(canonical(text(row[5])));
    ...
    await db.insert(usersTable).values({ id: teacherId, name: teacherName, email: teacherEmail && !usedTeacherEmails.has(teacherEmail) ? teacherEmail : null, role: "MENTOR", initials: initials(teacherName), department: "Computer Science & Engineering" }).onConflictDoNothing();
    if (teacherEmail) usedTeacherEmails.add(teacherEmail);
  ...
  for (const code of attendanceBook.SheetNames) {
    ...
    for (const row of sheetRows.slice(1)) {
      ...
      for (let column = 2; column < header.length; column += 1) {
        ...
        attendanceBatch.push({ id: id("attendance", `${student.admissionNo}:${subject.code}:${date}`), studentId, subjectId: id("subject", subject.code), sectionId: id("section", student.section), date, status: value === "P" ? "PRESENT" : "ABSENT", detail: "Imported from Attendance_Demo.xlsx" });
        report.attendance += 1;
